/*
package network.base.com.basedialog.circular;
...
import network.base.com.basedialog.DimmenUtils;
 ...
 * @author : cuu
 * date    : 2019/4/9
 * desc    : 圆形图片 小米手机mix3做圆角banner会出现兼容性问题
 *//*

public class CircularImages extends AppCompatImageView {
    ...
    public CircularImages(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CircularImages(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        this.context = context;

        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.circularimages, 0, 0);
        for (int i = 0; i < ta.getIndexCount(); i++) {
            int attr = ta.getIndex(i);
            if (attr == R.styleable.circularimages_is_cover_src) {
                isCoverSrc = ta.getBoolean(attr, isCoverSrc);
            } else if (attr == R.styleable.circularimages_is_circle) {
                isCircle = ta.getBoolean(attr, isCircle);
            } else if (attr == R.styleable.circularimages_border_width) {
                borderWidth = ta.getDimensionPixelSize(attr, borderWidth);
            } else if (attr == R.styleable.circularimages_border_color) {
                borderColor = ta.getColor(attr, borderColor);
            } else if (attr == R.styleable.circularimages_inner_border_width) {
                innerBorderWidth = ta.getDimensionPixelSize(attr, innerBorderWidth);
            } else if (attr == R.styleable.circularimages_inner_border_color) {
                innerBorderColor = ta.getColor(attr, innerBorderColor);
            } else if (attr == R.styleable.circularimages_corner_radius) {
                cornerRadius = ta.getDimensionPixelSize(attr, cornerRadius);
            } else if (attr == R.styleable.circularimages_corner_top_left_radius) {
                cornerTopLeftRadius = ta.getDimensionPixelSize(attr, cornerTopLeftRadius);
            } else if (attr == R.styleable.circularimages_corner_top_right_radius) {
                cornerTopRightRadius = ta.getDimensionPixelSize(attr, cornerTopRightRadius);
            } else if (attr == R.styleable.circularimages_corner_bottom_left_radius) {
                cornerBottomLeftRadius = ta.getDimensionPixelSize(attr, cornerBottomLeftRadius);
            } else if (attr == R.styleable.circularimages_corner_bottom_right_radius) {
                cornerBottomRightRadius = ta.getDimensionPixelSize(attr, cornerBottomRightRadius);
            } else if (attr == R.styleable.circularimages_mask_color) {
...
/**
    ...
    public void setBorderWidth(int borderWidth) {
        this.borderWidth = DimmenUtils.dp2px(context, borderWidth);
    ...
    public void setInnerBorderWidth(int innerBorderWidth) {
        this.innerBorderWidth = DimmenUtils.dp2px(context, innerBorderWidth);
        clearInnerBorderWidth();
        invalidate();
    }
    ...
    public void setCornerRadius(int cornerRadius) {
        this.cornerRadius = DimmenUtils.dp2px(context, cornerRadius);
        calculateRadiiAndRectF(false);
    }

    public void setCornerTopLeftRadius(int cornerTopLeftRadius) {
        this.cornerTopLeftRadius = DimmenUtils.dp2px(context, cornerTopLeftRadius);
        calculateRadiiAndRectF(true);
    }

    public void setCornerTopRightRadius(int cornerTopRightRadius) {
        this.cornerTopRightRadius = DimmenUtils.dp2px(context, cornerTopRightRadius);
        calculateRadiiAndRectF(true);
    }

    public void setCornerBottomLeftRadius(int cornerBottomLeftRadius) {
        this.cornerBottomLeftRadius = DimmenUtils.dp2px(context, cornerBottomLeftRadius);
        calculateRadiiAndRectF(true);
    }

    public void setCornerBottomRightRadius(int cornerBottomRightRadius) {
        this.cornerBottomRightRadius = DimmenUtils.dp2px(context, cornerBottomRightRadius);
        calculateRadiiAndRectF(true);
    }
